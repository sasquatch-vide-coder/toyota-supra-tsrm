...
export async function GET(request: NextRequest) {
  ...
  const sort = request.nextUrl.searchParams.get("sort") ?? "confidence";
  const limit = Math.min(parseInt(request.nextUrl.searchParams.get("limit") ?? "20", 10), 100);
  const offset = parseInt(request.nextUrl.searchParams.get("offset") ?? "0", 10);

  try {
    const { data, error } = await supabase.rpc("search_fixes", {
      search_query: q,
      filter_model: model,
    ...
    if (error) {
      console.error("search_fixes error:", error);
    ...
    console.error("Fixes search API error:", error);
    return NextResponse.json({ results: [], total: 0, error: "Search failed" }, { status: 500 });
  }
}
